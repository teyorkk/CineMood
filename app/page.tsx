...
export default function Home() {
  ...
  const [timeAvailable, setTimeAvailable] = useState<"short" | "standard" | "long" | "series" | undefined>(undefined);
    ...
    <div className="min-h-screen bg-[radial-gradient(100%_50%_at_50%_0%,rgba(120,119,198,0.25)_0,rgba(255,255,255,0)_50%)] dark:bg-[radial-gradient(100%_50%_at_50%_0%,rgba(120,119,198,0.15)_0,rgba(0,0,0,0)_50%)]">
      <main className="mx-auto max-w-5xl px-4 py-12">
        <header className="mb-8">
          <h1 className="text-3xl font-bold tracking-tight">CineMood</h1>
          <p className="text-sm text-zinc-600 dark:text-zinc-400 mt-1">Personalized movie & series picks based on your mood.</p>
        </header>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <Select
                ...
                onValueChange={(v) => setTimeAvailable((v === "any" ? undefined : (v as any)))}
                options={[
                  { value: "any", label: "Any" },
            ...
            <div className="flex items-end">
              <button
                ...
                onClick={() => getRecommendations({ mood, genres, contentType, timeAvailable })}
        ...
        {!mood || genres.length === 0 ? (
          <p className="mt-3 text-xs text-zinc-600 dark:text-zinc-400">Tip: select a mood and at least one genre to enable the button.</p>
        ) : null}
          ...
          {loading ? (
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
              {Array.from({ length: 8 }).map((_, i) => (
                <div key={i} className="rounded-2xl overflow-hidden border border-zinc-200 dark:border-white/10 bg-white/60 dark:bg-white/5">

...
export class TMDBClient {
  ...
  imageUrl(path: string | null | undefined, size: "w185" | "w342" | "w500" | "w780" | "original" = "w500"): string {
    if (!path) return "";
  ...
  async search(title: string, year?: number, type: TMDBMediaType = "movie") {
    ...
    const url = `${endpoint}?${this.qs({ query: title, year: type === "movie" ? year : undefined, first_air_date_year: type === "tv" ? year : undefined, include_adult: false, language: "en-US", api_key: this.apiKey })}`;
  ...
  async getDetails(id: number, type: TMDBMediaType) {
    ...
    const url = `${endpoint}?${this.qs({ language: "en-US", append_to_response: "credits,videos,release_dates,content_ratings,external_ids", api_key: this.apiKey })}`;
    const res = await fetch(url);
    if (!res.ok) throw new Error(`TMDB details failed: ${res.status}`);
    return res.json();
  }

  async getStreamingProviders(id: number, type: TMDBMediaType): Promise<StreamingPlatform[]> {
    ...
    for (const p of all) {
      ...
      if (!map.has(name)) {
        map.set(name, { name, logoPath: p.logo_path ? this.imageUrl(p.logo_path, "w185") : undefined });
      }
    }
    return [...map.values()];
  }
  ...
  trailerUrlFromVideos(videos: any): string | undefined {
    const yt = (videos?.results || []).find((v: any) => v.site === "YouTube" && v.type === "Trailer");

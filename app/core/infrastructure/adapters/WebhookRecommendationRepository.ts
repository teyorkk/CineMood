...
export class WebhookRecommendationRepository implements IRecommendationRepository {
  constructor(private webhook: N8NClient, private tmdb: TMDBClient) {}

  async generateRecommendations(request: RecommendationRequest): Promise<(Movie | Series)[]> {
    ...
    for (const rec of ai) {
      ...
      const castDetails = castArr.map((c: any) => ({
        ...
        profileUrl: c.profile_path ? this.tmdb.imageUrl(c.profile_path, "w185") : undefined,
      }));

        const computedYear = Number((type === "movie" ? details.release_date : details.first_air_date)?.slice(0, 4)) || rec.year || 0;
  ...
  castDetails,
        ...
        streamingPlatforms: await this.tmdb.getStreamingProviders(details.id, type),
        trailerUrl: this.tmdb.trailerUrlFromVideos(details.videos),
        imdbUrl: details?.external_ids?.imdb_id ? `https://www.imdb.com/title/${details.external_ids.imdb_id}/` : undefined,
           letterboxdUrl: `https://letterboxd.com/search/${encodeURIComponent(`${displayTitle} ${computedYear || ""}`)}/`,
      ...
      if (type === "movie") {
        ...
        const series: Series = { ...common, creator, seasons, episodeCount } as Series;
        out.push(series);
      }
    }
    return out;
  ...
  async fetchMovieDetails(movieId: string): Promise<Movie> {
    ...
    const cast = (credits.cast || []).slice(0, 5).map((c: any) => c.name).filter(Boolean);
    const director = (credits.crew || []).find((p: any) => p.job === "Director")?.name || "";
    return {
      id: String(d.id),
      title: d.title,
